...
const getBookingsByVenueIDAndDate = async (venueID, date) => {
    try {
        ...
        const filteredData = data.value.filter(row => (row.VENUE_ID === venueID && row.DATE === date));
...
const checkForOverlap = async (venueId, date, newStartTime, newEndTime) => {
    try {
...
export {createBooking, isOverlapping, getBookingsByVenueIDAndDate, checkForOverlap};
